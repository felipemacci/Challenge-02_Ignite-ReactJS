  ...
  switch(action.type) {
    case ActionTypes.ADD_NEW_ITEM:
      if(state.some(coffee => coffee.name === action.payload.name)) {
        ...
        return [...state, coffeeList[coffeeList.findIndex(coffee => coffee.name === action.payload.name)]]
      }
    case ActionTypes.REMOVE_ITEM:
      if(state.some(coffee => coffee.name === action.payload.name)) {
        if(state[state.findIndex(coffee => coffee.name === action.payload.name)].amount > 1) {
          const newCoffeeList: Coffee[] = state.map(coffee => {
          ...
          const newCoffeeList = state.filter(coffee => coffee.name !== action.payload.name)
